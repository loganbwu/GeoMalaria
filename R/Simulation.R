...
Simulation = R6Class(
  ...
  public = list(
    ...
    iterate = function(dt, debug=FALSE) {
      ...
      # Expose mosquitoes and add to current infected mosquitoes
      new_mosquito_infections = self$locations %>%
        ...
        mutate(infected_count = gametocyte_load *
                 mosquito_density * # local density could potentially be recomputed
                 self$bite_rate *
                 dt,
               infectious_count = 0, # gets overwritten later anyway but prevent NAs
               t_inoculation = self$t) %>%
        filter(infected_count > 0) %>% # remove redundant events
