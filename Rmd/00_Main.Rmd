...
p_infect = 0.2 # probability of infection for 1 mosquito/cell/day
```
...
iterate = function() {
  ...
  new_mosquito_infected <<- people %>%
    filter(infectious) %>%
    select(X, Y) %>%
    mutate(native_density = NA_real_,
           native_density = my_extract(mosquito_dens, .),
           t_event = t)
  ...
  people_infections = people %>%
    ...
    mutate(dispersed_density = native_density * infectivity * mosquito_migration(distance, t-t_event),
           log_p_noinfect = dispersed_density * log(1-p_infect),
    ) %>%
    group_by(ID) %>%
  ...
  step_plot <<- ggplot(plot_mosquitoes, aes(x=X, y=Y)) +
    ...
    coord_equal() +
    scale_fill_viridis_c(option="inferno", limits=c(0, 0.5)) +
